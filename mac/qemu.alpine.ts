...
namespace Static {
	export namespace OS {
		...
		export const Url = `https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/cloud/${Image}`;
	}
	...
	export namespace CloudInit {
		...
		export async function UserData() {
			const response = Util.Is.DEV
				? await $`ssh-add -L | grep "Personal Laptop"`.text()
				: await $.request(`https://github.com/navtoj.keys`).text();
			...
			const tool = [
				...
				{ package: 'containerd' },
				{ package: 'kubectl' },
				// https://pkgs.alpinelinux.org/packages?repo=main&arch=aarch64&branch=v3.23
			];
...
namespace Log {
	...
	export function debug(message: Message, fmt: Format = bgBrightGreen) {
		if (Util.Is.DEBUG) console.debug(fmt(parse(message)));
	}
...
namespace Zshrc {
	namespace prefix {
		...
		export const configLine =
			`alias ${Static.System.ID}='deno run --ext=ts --no-lock --allow-all --unstable-temporal "` as const;
	}
	namespace suffix {
	...
	export namespace Port {
		...
		export async function list() {
			...
			return lines.flatMap(line => {
				...
				return {
					pid: Type.State.shape.pid.parse(pid),
					local: Type.Port.parse(local),
					remote: Type.Port.parse(remote),
				};
			});
		}
...
const stale =
	!Util.Is.DEV && import.meta.url !== (await Zshrc.scriptUrl(args.install));
...
switch (
	args._.length === 1 || (args._.length === 2 && args._[0] === 'port')
		? args._[0]
		: null
) {
	case 'start': {
	...
	case 'port': {
		...
		if (input === null) {
			...
			if (!tunnels.length)
				Log.exit(Block.lined('No active tunnels found.'), yellow);
			Log.exit(
				[
					Block.boxed(bold('Port Tunnels'), { bold: true }),
					tunnels.map(p =>
						Block.lined(
							`${p.local} ──⏵ ${p.remote} ${dim(`(${p.pid})`)}`,
						),
					),
				].flat(),
			);
		...
		const message = Block.lined([
			result === null
				...
				: result
					? `Tunnel ${negative ? 'closed' : 'opened'} to port.`
					: `Failed to ${negative ? 'close' : 'open'} tunnel.`,
		]);

		if (result === false) Log.quit(message);
		else Log.info(message, result === null ? yellow : green);
		break;
	}
	...
	default: {
		...
		const message = [
			...
			Block.lined([
				`Usage: ${Static.System.ID} <command>`,
				'Commands: start | stop | reset | port | help',
			]),
		];
		if (status === false)
			message.push(
